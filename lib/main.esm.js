class ObjectDescription {
    constructor(target, options) {
        ...
        const { formatter, style, styles, nodeMultiple, comment, descriptor } = this.options;
        if (formatter) {
        ...
        if (descriptor) {
            for (let key in descriptor) {
                INFO_MAP_PATH[key] && (INFO_MAP_PATH[key].desc = descriptor[key]);
            }
        }
    }
...
export { ObjectDescription as default };
