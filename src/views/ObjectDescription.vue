<template>
    ...
    <div class="righter">
        <div class="theme">可选预览</div>
        <div class="theme-container">
            <div class="item" v-for="item in nodeOptions" :key="item.title">
                <span class="title">{{item.title}}</span>
                <span class="option" v-for="option in item.options" :key="option.index" @click="lightNode(option)">{{option.name}}</span>
            </div>
        </div>
        <div class="theme">预设组合</div>
        <div class="theme-container">todo</div>
        <div class="theme">筛选输出</div>
        <div class="theme-container">todo</div>
    </div>
</template>
...
export default {
    setup() {
    const html = ref('123456') // 变量repositories = []是非响应式
    ...
    nodeOptions.value = objhtml.nodeMapOptions
    console.log(objhtml.nodeMapOptions);
